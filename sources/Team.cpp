...
namespace ariel
{
    ...
    void Team::sortByType()
    {
        ...
        {
            if (isCowboy(members[i]))
            {
                cowboys.push_back(static_cast<Cowboy *>(members[i]));
            }
            else if (isNinja(members[i]))
            {
                ninjas.push_back(static_cast<Ninja *>(members[i]));
            }
        }
